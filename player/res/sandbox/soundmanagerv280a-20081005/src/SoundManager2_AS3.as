...
public class SoundManager2_AS3 extends Sprite {
  ...
  public function _load(sID:String,sURL:String,bStream:Boolean,bAutoPlay:Boolean):void {
    ...
    if (s.didLoad == true) {
      ...
      _destroySound(s.sID);
      _createSound(ns.sID,sURL,ns.justBeforeFinishOffset,ns.usePeakData,ns.useWaveformData,ns.useEQData,ns.useNetstream);
      s = soundObjects[sID];
    ...
    if (s.useNetstream) {
	  // s.ns.close();
	  s.ns.addEventListener(AsyncErrorEvent.ASYNC_ERROR, function(e:AsyncErrorEvent):void{doAsyncError(s,e)});
	  s.ns.addEventListener(NetStatusEvent.NET_STATUS, function(e:NetStatusEvent):void{doNetStatus(s,e)});
	  s.ns.addEventListener(IOErrorEvent.IO_ERROR, function(e:IOErrorEvent):void{doIOError(s,e)});
	  s.ns.play(sURL);
	} else {
      try {
  ...
  public function _createSound(sID:String,sURL:String,justBeforeFinishOffset:int,usePeakData:Boolean,useWaveformData:Boolean,useEQData:Boolean,useNetstream:Boolean):void {
    soundObjects[sID] = new SoundManager2_SMSound_AS3(this,sID,sURL,usePeakData,useWaveformData,useEQData,useNetstream);
    var s:SoundManager2_SMSound_AS3 = soundObjects[sID];
    // s.setVolume(100);
    s.didJustBeforeFinish = false;
    s.sID = sID;
