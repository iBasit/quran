...
*/
    function get_url(index) {
        ...
        function get_mirror() {
            return $.quran.config.mp3_mirrors[0];
        }
        function get_recitor() {
            return $.quran.get_state('recitor');
        }
    ...
    quran.onaya = function() {
        var ayas = smUtils.getElementsByClassName('aya','option');
        $.each(ayas, function(i,item) {
            self.links[self.links.length] = item;
            self.addItem({ url: get_url(item.value) });
        });
        self.createPlaylist();
        self.createTweens(); // make tweens for playlist
        self.loadFromHash();
    };
    /*
...
function SPPLaylistItem(oLink,oPlaylist,nIndex) {
  ...
  this.onclick = function() {
    if (oParent.doShuffle) quran.player.toggleShuffle(); // disable shuffle, if on (should be oParent.oParent too, ideally)
...
function SoundPlayer() {
  ...
  this.onload = function() {
    ...
    soundManager._writeDebug('quran.player.onload()');
  ...
  this.toggleMute = function() {
    // soundManager._writeDebug('quran.player.toggleMute()');
    self.oSMPlayer.toggleMute();
  }

  this.togglePlaylist = function() {
    soundManager._writeDebug('quran.player.togglePlaylist()');
...
quran.player = new SoundPlayer();

function initStuff() {
  quran.player.init(); // load mp3, etc.
  setTimeout(initOtherStuff,20);
}

function initOtherStuff() {
  quran.player.oPlaylist = new SPPlaylist(quran.player,null);
  quran.player.oPlaylist.searchForSoundLinks();
}
